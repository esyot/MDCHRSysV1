...
export default {
  ...
  methods: {
    ...
    fetchTraveledDays(term) {
      ...
      if (term === '1st') {
        count = this.forms.filter(form => 
          form.semister === '1st' && new Date(form.date_start).getFullYear() === currentYear
        ).reduce((total, form) => total + this.calculateDays(form.date_start, form.date_end), 0);
      } else if (term === '2nd') {
        count = this.forms.filter(form => 
          form.semister === '2nd' && new Date(form.date_start).getFullYear() === currentYear
        ).reduce((total, form) => total + this.calculateDays(form.date_start, form.date_end), 0);
      } else if (term === 'summer') {
        count = this.forms.filter(form => 
          form.semister === 'summer' && new Date(form.date_start).getFullYear() === currentYear
        ).reduce((total, form) => total + this.calculateDays(form.date_start, form.date_end), 0);
      }
