<template>
  <div class="personal-details-items" v-if="editMode">
    ...
    >
      <div class="form-group">
        <label for="title"
          >TITLE OF TRAINING/SEMINARS/CONFERENCES ATTENDED (Write in Full):</label
        >
        <input
          type="text"
          id="title"
      ...
      <div class="form-group">
        <label for="conducted_by">CONDUCTED/SPONSORED BY (Write in Full):</label>
        <input
          type="text"
          name="conducted_by"
          id="conducted_by"
  ...
  <div class="personal-details-items" v-if="editMode == false">
    <div class="title-container">
      <span class="title"
        >SPECIAL TRAINING COURSES, SEMINARS, SYMPOSIA PARTICIPATED (Last 5 Years)</span
      >
    </div>
    <div
    ...
    >
      ...
      <span>
        ...
        {{ userSpecialTraining.attendance_to }} || {{ userSpecialTraining.days }} ||
...
export default {
  ...
  watch: {
    personalDetails: {
      handler(newVal) {
        this.userDetails.user_special_trainings = newVal.user_special_trainings || [];
      },
      immediate: true,
    },
  },
